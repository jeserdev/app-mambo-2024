...
const OrderMainScreenCutThree = () => {
    ...
    const unsubscribe = onValue(orderRef, (snapshot) => {
      ...
      if (data) {
        let orders = Object.keys(data)
          ...
          .filter((order) => order.orderCut === 3) // Filtrar por orderCut: 3

        // Verificar si hay pedidos pendientes antes de filtrar (basado en filteredOrders)
        ...
        // Filtrar por usuario (solo para clientes)
        if (userData.role === 'Cliente') {
          orders = orders.filter(
            (order) => order && order.userId === userData.uid
          )
        }
  ...
  const handleSendOrders = async () => {
    try {
      ...
      setFilteredOrders(updatedOrders.filter((order) => order.orderCut === 3))
      ...
      ListHeaderComponent={
        <>
          <View style={styles.header}>
            <Text style={styles.title}>Pedido General corte Three</Text>
          </View>
...
export default OrderMainScreenCutThree
