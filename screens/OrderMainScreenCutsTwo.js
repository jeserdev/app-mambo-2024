...
const OrderMainScreenCutTwo = () => {
    ...
    const unsubscribe = onValue(orderRef, (snapshot) => {
      ...
      if (data) {
        let orders = Object.keys(data)
          ...
          .filter((order) => order.orderCut === 2) // Filtrar por orderCut: 3
  ...
  const handleSendOrders = async () => {
    try {
      ...
      setFilteredOrders(updatedOrders.filter((order) => order.orderCut === 2))
      ...
      ListHeaderComponent={
        <>
          <View style={styles.header}>
            <Text style={styles.title}>Pedido General corte Two</Text>
          </View>
...
export default OrderMainScreenCutTwo
